...
function CH:PositionChat(override)
	...
	for i=1, CreatedFrames do
		...
		if point == "BOTTOMRIGHT" and chat:IsShown() and not (id > NUM_CHAT_WINDOWS) and id == self.RightChatWindowID then
			...
			if id ~= 2 then
				chat:SetSize(E.db.chat.rightpanelWidth - 11, (E.db.chat.rightpanelHeight - BASE_OFFSET))
			else
				chat:Size(E.db.chat.rightpanelWidth - 11, (E.db.chat.rightpanelHeight - BASE_OFFSET) - CombatLogQuickButtonFrame_Custom:GetHeight())				
			end		
		...
		elseif not isDocked and chat:IsShown() then
			...
			if id ~= 2 and not (id > NUM_CHAT_WINDOWS) then
				...
				chat:Size(E.db.chat.leftpanelWidth - 11, E.db["euiscript"].chatbar and (E.db.chat.leftpanelHeight - BASE_OFFSET - 26) or (E.db.chat.leftpanelHeight - BASE_OFFSET))
				FCF_SavePositionAndDimensions(chat)		
			end
...
local joinChannelFunc = function(channel)
    ...
    CH:UnregisterEvent("CHAT_MSG_CHANNEL_NOTICE")

    local worldChannels = CH.worldChannels
    local joinOne = false
...
function CH:CHAT_MSG_CHANNEL_NOTICE(...)
...
function CH:PLAYER_ENTERING_WORLD(...)
    ...
    hooksecurefunc("ChatFrame_AddChannel", function(chatframe, name)
        if name==L["BigFootChannel"] then
            self.worldChannels = self.worldChannels or {}
            self.worldChannels[_G[chatframe:GetName().."Tab"]:GetText()] = 1
        end
    end)
    hooksecurefunc("ChatFrame_RemoveChannel", function(chatframe, name)
        if name==L["BigFootChannel"] then
            self.worldChannels = self.worldChannels or {}
            self.worldChannels[_G[chatframe:GetName().."Tab"]:GetText()] = nil
        end
    end)
