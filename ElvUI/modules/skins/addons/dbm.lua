	...
	for bar in self:GetBarIterator() do
		if not bar.injected then
				...
				if not name.styled then
					...
					name:SetWidth(165)
					name:SetHeight(8)
					name:FontTemplate(nil, E.private.skins.dbm.fontsize, 'OUTLINE')
				...
				if not timer.styled then	
					...
					timer:FontTemplate(nil, E.private.skins.dbm.fontsize, 'OUTLINE')
	...
	if not anchor.styled then
		local header={anchor:GetRegions()}
			if header[1]:IsObjectType("FontString") then
				header[1]:FontTemplate(nil, E.private.skins.dbm.fontsize, 'OUTLINE')
...
local SkinInfoFrame = function()
	while DBMInfoFrame and DBMInfoFrame.skined ~= true do
		DBMInfoFrame:SetTemplate("Transparent")
		DBMInfoFrame.skined = true
	end
end
