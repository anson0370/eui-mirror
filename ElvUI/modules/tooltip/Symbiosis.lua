...
GameTooltip:HookScript("OnTooltipSetUnit", function(self)
	...
	if unit then
		if UnitIsPlayer(unit) and not UnitIsEnemy(unit, "player") then
			...
			local spellID
			if E.myclass == "DRUID" and UnitLevel("player") >= 87 and uclass ~= "DRUID" then
				spellID = symbiosis.grant[uclass][spec]
			elseif E.myclass ~= "DRUID" and (uclass == "DRUID" and ulevel >= 87) then
				spellID = symbiosis.gain[E.myclass][spec]
			end
