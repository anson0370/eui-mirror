...
function TT:SetStyle(tt)
	if not tt.backdropTexture then
		tt:SetTemplate(E.db.tooltip.transparent and "Transparent" or "notrans")
...
function TT:PLAYER_ENTERING_WORLD()
	if not self.initialhook then
		...
		FriendsTooltip:SetTemplate(E.db.tooltip.transparent and "Transparent" or "notrans")
...
function TT:GameTooltip_OnTooltipSetUnit(tt)
	...
	local iLevel, talentSpec, lastUpdate, role = 0, "", 30, ""
		...
		if guildName then
			if E.db.tooltip.guildranks then
				if UnitIsInMyGuild(unit) then
					GameTooltipTextLeft2:SetText("<"..E["media"].hexvaluecolor..guildName.."|r> [".. guildRankIndex.. "] ["..E["media"].hexvaluecolor..guildRankName.."|r]")
				else
					GameTooltipTextLeft2:SetText("<|cff00ff10"..guildName.."|r> [".. guildRankIndex.. "] [|cff00ff10"..guildRankName.."|r]")
				end
			else
				if UnitIsInMyGuild(unit) then
	...
	if E.db.tooltip.ilevel and (iLevel > 1) then
	...
	if E.db.tooltip.talent and talentSpec ~= "" and role ~= "" then
		GameTooltip:AddDoubleLine(TALENT..":", talentSpec.. " - ".. _G[role])
	end
...
function TT:INSPECT_READY(event, GUID)
	...
	local talentSpec, role = TT:GetTalentSpec('mouseover')
