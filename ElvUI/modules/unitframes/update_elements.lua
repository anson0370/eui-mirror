...
function UF:PostCastStart(unit, name, rank, castid)
	...
	if db.castbar.ticks and unit == "player" then
		...
		if baseTicks and unitframe.ChannelTicksSize[name] and E.global.unitframe.HastedChannelTicks[name] then
	...
	if self.interrupt and unit ~= "player" then
		...
		self:SetStatusBarColor(colors.castColor[1], colors.castColor[2], colors.castColor[3])
		if db.castbar.InterruptSound and CanInterrupt() and UnitCanAttack("player", unit) then PlaySoundFile(LSM:Fetch("sound", "Kick Cast"), "Master") end;
	end
end
