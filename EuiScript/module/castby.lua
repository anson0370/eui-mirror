...
if E.db["euiscript"].castby ~= true then return end
--[[
...
local cc = {}
local CUSTOM_CLASS_COLORS = RAID_CLASS_COLORS
do
...
local function SetCaster(self, unit, index, filter)
	...
	if unitCaster then
		...
		self:AddLine("cast by " .. (cc[uclass] or '|cffffffff') .. uname .. '|r (' .. unitCaster .. ')')
		self:Show()
	end
end
