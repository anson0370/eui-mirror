...
if E.db.chatfilter.chatfilterbylevel == 0 then return; end
----------------------------------------------------
--1级小号屏蔽(从蘑菇整合包中提取)
...
do
	...
	elseif L == "zhTW" then
		whisp = "你起碼要達到 %d 級才能密我。"
		err = "你的好友列表滿了，此插件需要你騰出2個好友空位!"
	elseif L == "zhCN" then
		whisp = "你起码要达到 %d 级才能和我讲话"
		...
		for i =  num,1,-1 do
			...
			if not player then
				...
				if maybe[player] then --do we need to process this person?
					...
					if level < (filterTable[player] or (E.db.chatfilter.chatfilterbylevel + 1)) then
				...
				elseif friendcatch[player] then
					filterTable[player]  = E.db.chatfilter.chatfilterbylevel + 1
...
local function filter(...)
	...
	local level = E.db.chatfilter.chatfilterbylevel + 1
