...
FriendsMenuXP_Buttons["Chat_BlankName"] = {
	text = FMXP_BUTTON_CHAT_BLACKLIST,
	show = function(name)
		if unpack(ElvUI).global.chatfilter.BlankName[name] then
			return false;
		else
			return true;
		end
	...
	func = function(name, dropdownFrame)
		unpack(ElvUI).global.chatfilter.BlankName[name] = true
		DEFAULT_CHAT_FRAME:AddMessage(name.. FMXP_BUTTON_CHAT_BLACKLIST_INTRO,1,1,0);
		for k, v in pairs(unpack(ElvUI).global.chatfilter.BlankName) do
			unpack(ElvUI).Options.args.chatfilter.args.BlankName.args.List.values[k] = k
		end		
	end,
}
